...
try {
    ...
    foreach ($processItems as $item) {
        ...
        // 2. Hard character limit: max 200 chars (roughly 2 short sentences)
        // Split on commas and periods to find natural break points
        if (!preg_match('/\[VOICE/', $reply) && !preg_match('/\[PPV/', $reply)) {
            if (mb_strlen($reply) > 200) {
                // Find last good break point before 200 chars
                $truncated = mb_substr($reply, 0, 200);
                // Try to break at sentence end
                $lastPeriod = max(strrpos($truncated, '.'), strrpos($truncated, '!'), strrpos($truncated, '?'));
                if ($lastPeriod && $lastPeriod > 50) {
                    $reply = mb_substr($reply, 0, $lastPeriod + 1);
                } else {
                    // Break at last comma
                    $lastComma = strrpos($truncated, ',');
                    if ($lastComma && $lastComma > 50) {
                        $reply = mb_substr($reply, 0, $lastComma);
                    } else {
                        $reply = $truncated;
                    }
                }
                $debug[] = "hard_truncated";
            }
        }
