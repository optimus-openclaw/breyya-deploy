...
try {
    ...
    foreach ($processItems as $item) {
        ...
        if (!$ppvBlocked && preg_match('/\[PPV:([^:]+):([0-9.]+)\]/', $reply, $ppvMatches)) {
            $ppvSetTierId = trim($ppvMatches[1]); // Now set_id_tier format
            $ppvPriceCents = intval($ppvMatches[2]); // Now in cents
            ...
            if (file_exists($inventoryFile)) {
                ...
                // Find the requested set and tier (v2 format)
                $ppvSet = null;
                $ppvTier = null;
                
                // Parse set_id_tier format (e.g., "2020-09-14_pink-bra_nonnude")
                $parts = explode('_', $ppvSetTierId);
                if (count($parts) >= 3) {
                    $tier = array_pop($parts); // Last part is tier
                    $setId = implode('_', $parts); // Everything else is set_id
                    
                    // Find the requested set and tier
                ...
                if ($ppvSet && $ppvTier && $ppvPriceCents > 0) {
                    $tierData = $ppvSet['tiers'][$ppvTier];
                    
                    // Generate preview — prefer images over videos for thumbnail
                    $previewUrl = '';
                    $imageFiles = array_values(array_filter($tierData["files"] ?? [], function($f) { return $f["type"] === "image"; }));
                    $videoFiles = array_values(array_filter($tierData["files"] ?? [], function($f) { return $f["type"] === "video"; }));
                    $allItems = array_map(function($f) { return $f["url"]; }, $tierData["files"] ?? []);
                    $firstItem = !empty($imageFiles) ? $imageFiles[0]["url"] : (!empty($allItems) ? $allItems[0] : '');
                    $isVideoOnly = empty($imageFiles) && !empty($videoFiles);
                    if (!empty($allItems)) {
                        
                        // For images, try to create blurred preview
                        if (strpos($firstItem, '.jpg') !== false || strpos($firstItem, '.jpeg') !== false || strpos($firstItem, '.png') !== false) {
                            $previewDir = __DIR__ . '/../../data/ppv-previews/';
                            if (!is_dir($previewDir)) {
                                mkdir($previewDir, 0755, true);
                            }
                            
                            $previewFile = $previewDir . $ppvSetTierId . '_preview.jpg';
                            if (function_exists('imagecreatefromjpeg') && !file_exists($previewFile)) {
                                // Create blurred preview using GD
                                $imageData = @file_get_contents($firstItem);
                                if ($imageData) {
                                    ...
                                    if ($sourceImage) {
                                        ...
                                        $previewUrl = '/data/ppv-previews/' . $ppvSetTierId . '_preview.jpg';
                                    }
                                }
                        ...
                        // Fallback: for video-only sets, use a video placeholder; otherwise use original URL
                        if (!$previewUrl) {
                            if ($isVideoOnly) {
                                $previewUrl = '/images/video-preview-placeholder.jpg';
                            } else {
                                $previewUrl = $firstItem;
                            }
                        }
                    }
                    ...
                    // Insert the PPV message with set info
                    $safePpvContentKey = $db->escapeString($ppvSetTierId); // Store set_id_tier for unlock
                    $safePpvPreviewUrl = $db->escapeString($previewUrl);
                    // Prefer first image for media_url (frontend renders as <img>); videos only if no images
                    $ppvMediaItem = !empty($imageFiles) ? $imageFiles[0]["url"] : ($firstItem ?: 'N/A');
                    $safePpvMediaUrl = $db->escapeString($ppvMediaItem);
                    ...
                    $db->exec("INSERT INTO messages (sender_id, receiver_id, content, media_url, media_thumbnail, is_ppv, ppv_price_cents, is_unlocked, ppv_preview_url, ppv_content_key, is_ai, created_at) 
                              VALUES ($CREATOR_ID, $fid, 'Exclusive content set - unlock to view 🔒', '$safePpvMediaUrl', '$safePpvPreviewUrl', 1, $ppvPriceCents, 0, '$safePpvPreviewUrl', '$safePpvContentKey', 1, datetime('now', '+1 second'))");
