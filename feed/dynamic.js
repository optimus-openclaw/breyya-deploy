(function(){
  ...
  function buildPost(p) {
    ...
    article.dataset.liked = p.liked ? '1' : '0';
    ...
    article.innerHTML =
      '<div class="feed-module__Sej6XW__postHeader">' +
        ...
        '<span class="feed-module__Sej6XW__postTime">'+fmt(p.created_at)+'</span></div>' +
      '</div>' +
      ...
      '<div class="feed-module__Sej6XW__postActions">' +
        '<button class="feed-module__Sej6XW__actionBtn heart-btn" data-liked="'+(p.liked?'1':'0')+'" style="cursor:pointer;transition:transform 0.2s">' +
        ...
        '</a>' +
        '<button class="feed-module__Sej6XW__actionBtn tip-btn" style="cursor:pointer">' +
    ...
    hb.addEventListener('click', function(e) {
      ...
      fetch('/api/posts/like.php',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json','Authorization':'Bearer '+getToken()},body:JSON.stringify({post_id:parseInt(p.id)})}).catch(function(){});
    });

    // Wire tip
    article.querySelector('.tip-btn').addEventListener('click', function(e) {
      e.preventDefault(); e.stopPropagation();
      showTip();
    });
  ...
  function showTip() {
    ...
    ov.setAttribute('style','position:fixed;inset:0;background:rgba(0,0,0,0.7);z-index:99999;display:flex;align-items:center;justify-content:center;');
  ...
  // Simple, aggressive loading - try multiple times
  loadFeed();
  setTimeout(loadFeed, 500);
  setTimeout(loadFeed, 1500);
  setTimeout(loadFeed, 3000);
  document.addEventListener('DOMContentLoaded', loadFeed);
  window.addEventListener('load', function() { setTimeout(loadFeed, 500); });
  document.addEventListener('visibilitychange', function() {
    if (document.visibilityState === 'visible') setTimeout(loadFeed, 200);
  });

  // Watch for React to render the feed container
  var obs = new MutationObserver(function() {
    if (document.querySelector('.feed-module__Sej6XW__feed') && !document.getElementById('dynamic-posts')) {
      loadFeed();
    }
  });
